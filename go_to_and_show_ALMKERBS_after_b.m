function norm_of_DmR = go_to_and_show_ALMKERBS_after_b(An,Ln,Mn,Kn,En,Rn,Bn,Sn,DQ,RQ)
...
plot(f_temp,delta_dmr,'b');
ylim([-50 50]);
hold on;
%plot(f_temp,V_M_fr/V_M_f,'r');
hold off;
xlabel('frequency [Hz]');
ylabel('intensity [dB]');
legend('D-R','D/R');
title(['A' num2str(Ant),' L' num2str(Lnt),' M' num2str(Mnt),' K' num2str(Knt),' E' num2str(Ent),' R' num2str(Rnt),' B' num2str(Bnt),' S' num2str(Snt)]); % if this line is a problem just delete it
norm_of_DmR=norm(delta_dmr(1:140));
